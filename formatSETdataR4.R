...
formatSETdataR4<-function(dataIn, dataDir){
  ...
  position.df<-as.data.frame(unique(data.1[, c("Plot_Name","EventDate","PipeDirectionAzimuth")]))
  ...
  data.1<-merge(data.1, positionFactor.save, by=c("Plot_Name","EventDate","PipeDirectionAzimuth"), all.x=TRUE)
  ...
  data.2$station.position.year<-paste(data.2$Plot_Name, data.2$Position_Name, data.2$Year,sep=".")
  ...
  #melt function .
    data.melt<-reshape2::melt(data.out.keep, id=c("RefugeName","Plot_Name","ReaderFullName",
                                 "Year","Position_Name","PipeDirectionAzimuth","PinPosition","Visit","year.visit"),
                    measure=c("PinHeight_mm"))

    # colnames(data.melt)<-c("Plot_Name","ReaderFullName","Year","Position","Visit","year.visit","Pin","Value")
...
data.out.keep.1<-unique(data.out.keep[,c("RefugeName","Site_Name","Plot_Name","EventDate","Year","Month","Day","ord.Day", "Visit","year.visit","ReaderFullName","ReaderID","PipeDirectionAzimuth","Position_Name","Plot_Year_Visit_Position")])
...
new.data.out.2<-new.data.out.1
  ...
  #Rename columns.
  colnames(set.delta.data)<-c("State","RefugeName","Site_Name","Plot_Name","Date","EventDate","Year","Month","Day","ord.Day", "Visit","year.visit","ReaderFullName","ReaderID","PipeDirectionAzimuth","Position_Name","Plot_Year_Visit_Position","Longitude","Latitude","Pin1","Pin2","Pin3","Pin4","Pin5","Pin6","Pin7","Pin8","Pin9")
  
  #Melt data to stack all Pins (Pin1, Pin2, . . .) in one column.
  delta.set.melt<-reshape2::melt(set.delta.data, id=c("State","RefugeName","Site_Name","Plot_Name","Date","EventDate","Year","Month","Day","ord.Day", "Visit","year.visit","ReaderFullName","ReaderID","PipeDirectionAzimuth","Position_Name","Plot_Year_Visit_Position","Longitude","Latitude"),
